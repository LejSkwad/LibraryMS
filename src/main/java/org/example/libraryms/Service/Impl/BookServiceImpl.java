...
public class BookServiceImpl implements BookService {
    ...
    public Page<BookSearchResponse> search(BookSearchRequest bookSearchRequest, Pageable pageable) {
        ...
        if (bookSearchRequest.getCategoryId() != null) {
            spec = spec.and(BookSpecification.categoryIdEqual(bookSearchRequest.getCategoryId()));
        }
        if (bookSearchRequest.getPublisher() != null && !bookSearchRequest.getPublisher().isBlank()) {
            spec = spec.and(BookSpecification.publisherEqual(bookSearchRequest.getPublisher()));
        }
        ...
        return bookRepository.findAll(spec, pageable).map(bookMapper::toSearchResponse);
    }

    @Override
    @Transactional
    public void create(BookCreateRequest bookCreateRequest) {
        if (bookRepository.existsByTitleContainingIgnoreCaseAndAuthorContainingIgnoreCaseAndPublishedYear(
                bookCreateRequest.getTitle(), bookCreateRequest.getAuthor(), bookCreateRequest.getPublishedYear())) {
