...
public class CategoryController {
    ...
    public ResponseEntity<BaseResponse<List<CategoryResponse>>> getCategory() {
        ...
        return ResponseEntity.status(HttpStatus.OK).body(new BaseResponse<>(data, "Lấy thể loại thành công"));
    }

    @PostMapping("/v1/category")
    public ResponseEntity<BaseResponse<Void>> create(@RequestParam String name) {
        categoryService.create(name);
        return ResponseEntity.status(HttpStatus.OK).body(new BaseResponse<>(null, "Thêm Thể loại thành công"));
    }

    @DeleteMapping("/v1/category/{id}")
    public ResponseEntity<BaseResponse<Void>> delete(@PathVariable Integer id) {
        categoryService.delete(id);
        return ResponseEntity.status(HttpStatus.OK).body(new BaseResponse<>(null, "Xóa Thể loại thành công"));
    }
}
